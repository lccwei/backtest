...
protected:
    ...
    QByteArray
    cacheKey(const QString& text, const QColor& color, double rotation, AnchorSide side) const;
    AnchorSide
    skewedAnchorSide(const QPointF& tickPos, double sideExpandHorz, double sideExpandVert) const;
...
protected:
    ...
    virtual QString
    getTickLabel(double tick, const QLocale& locale, QChar formatChar, int precision);
...
void QCPDataContainer<DataType>::add(const QCPDataContainer<DataType>& data) {
    ...
    } else  // don't need to prepend, so append and merge if necessary
    {
        ...
        if (oldSize > 0
            && !qcpLessThanSortKey<DataType>(
                *(constEnd() - n - 1),
                *(constEnd() - n)))  // if appended range keys aren't all greater than existing
                                     // ones, merge the two partitions
...
void QCPDataContainer<DataType>::performAutoSqueeze() {
        ...
        > 650000)  // if allocation is larger, shrink earlier with respect to total used size
    {
        shrinkPostAllocation =
            postAllocSize > usedSize * 1.5;  // QVector grow strategy is 2^n for static data. Watch
...
protected:
    ...
    // non-virtual methods:
    double
    getTangentAngle(const QCPPlottableInterface1D* interface1d, int dataIndex, int direction) const;
...
public:
    ...
    */
    ...
    void
    setData(const QVector<double>& keys, const QVector<double>& values, bool alreadySorted = false);
    ...
    // non-property methods:
    void
    addData(const QVector<double>& keys, const QVector<double>& values, bool alreadySorted = false);
...
protected:
    ...
    // non-virtual methods:
    void
    getCurveLines(QVector<QPointF>* lines, const QCPDataRange& dataRange, double penWidth) const;
...
protected:
    ...
    // non-virtual methods:
    QLineF
    getRectClippedLine(const QCPVector2D& start, const QCPVector2D& end, const QRect& rect) const;
    QPen mainPen() const;
};
